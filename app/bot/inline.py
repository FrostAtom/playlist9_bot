...
from ..music.resolver import ClassifiedInput, InputKind
...
async def answer(deps: Deps, query: InlineQuery) -> None:
    ...
    # Classify once with the shared resolver (same decision tree as on_text).
    classified = resolver.classify(deps.service, text)

    # A TikTok link is a video, not a track — offer the clip as its own result.
    if classified.kind is InputKind.TIKTOK:
        await query.answer(
            [_tiktok_result(deps, classified.tiktok_url)], cache_time=0, is_personal=True
        )
        return

    try:
        tracks = await _resolve_tracks(deps, classified, limit)
...
async def _resolve_tracks(
    deps: Deps, info: ClassifiedInput, limit: int
) -> List[Track]:
    """Turn a classified inline query into a list of downloadable tracks.

    Everything except TikTok (handled by the caller) collapses to "a list of
    Tracks": a search, a single pasted track link, or the contents of a pasted
    playlist/album link."""
    kind = info.kind
    ...
    # Plain text search.
    source = deps.service.default_source()
    return await deps.service.search(info.text, limit, source)
